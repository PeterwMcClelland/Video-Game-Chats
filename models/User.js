...
// Create User model
class User extends Model { // Method to validate access
    checkPassword(loginPwd) {
        return bcrypt.compareSync(loginPwd, this.password);
    }
};
// Define user table
User.init({ // Define id column
    id: {
    ...
    password: {
        type: DataTypes.STRING,
        allowNull: false,
        validate: { // Define minimun length to password, set in 6
            len: [6]
        }
    }
}, {
    // Methods to hash password
    hooks: { // Method when create a new user hash password
